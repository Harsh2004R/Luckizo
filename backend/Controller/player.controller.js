...
export const createPlayerController = async (req, res) => {
  ...
  try {
    ...
    if (existingPlayer) {
      return res
        .status(401)
        .json({ message: "Player is already exist in database ..." });
    }
...
export const getPlayerInfoController = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "player details fetched...", player });
  } catch (error) {
    return res.status(400).json({
      message: "Error in comming from get user info api...",
      error: error.message,
    });
  }
};
...
export const editPlayerController = async (req, res) => {
  const { id } = req.params;
  try {
    const updatedPlayer = await PlayerModel.findByIdAndUpdate(id, req.body, {
      new: true,
    });

    console.log({ updatedPlayer });
    return res.status(200).json({
      message: "Player Profile is updated successful ...",
